...
Validator.messageSource['en-us'] = [
    ...
    ['validate-resident-code','Please enter a valid resident code']
]
...
Validator.messageSource['zh-cn'] = [
    ...
    ['validate-resident-code','请输入有效的工作居住证编码']
]
...
Validation.addAllThese([
    ...
    ['validate-resident-code',function(v,elm,args,metadata) {
        if(!(/^\d{12}$/.test(v))) return false;
        var birthday = v.substr(0,8);
        if(!ValidationUtils.isDate(birthday,'yyyyMMdd')) return false;
        return true;
    }]
]);
